#!/usr/bin/env python3
"""
Enhanced Exposure Validation Analysis - Fixed Version
Author: Ryhan Suny
Date: 2025-05-25

This script analyzes both AND and OR logic exposure definitions with improved visualizations.
"""
...
def create_exposure_comparison_chart(data, output_dir):
    """Create improved exposure comparison visualization"""
    ...
    ax1.pie([or_exposed, or_unexposed], 
            labels=[f'Exposed\n{or_exposed:,}\n({or_exposed/total*100:.1f}%)',
                   f'Unexposed\n{or_unexposed:,}\n({or_unexposed/total*100:.1f}%)'],
            colors=['#FF6B6B', '#4ECDC4'],
            startangle=90,
            textprops={'fontsize': 11})
    ax1.set_title('OR Logic (Any Criterion)', fontsize=14, fontweight='bold')
    
    # 2. AND Logic - use bar chart (top right)
    ...
    # 3. Direct comparison (bottom left)
...
OR Logic:
• Exposed: {or_exposed:,}
• Rate: {or_exposed/total*100:.1f}%

AND Logic:
• Exposed: {and_exposed:,}
• Rate: {and_exposed/total*100:.2f}%
...
def create_criteria_venn_diagram(data, output_dir):
    """Create improved Venn diagram with bar chart alternative"""
    # First create the Venn diagram
...
━━━━━━━━━━━━━━━━━━
OR Logic: {len(h1 | h2 | h3):,} ({len(h1 | h2 | h3)/len(data)*100:.1f}%)
AND Logic: {len(h1 & h2 & h3):,} ({len(h1 & h2 & h3)/len(data)*100:.2f}%)

Individual Criteria:
• H1 only: {len(h1 - h2 - h3):,}
• H2 only: {len(h2 - h1 - h3):,}
• H3 only: {len(h3 - h1 - h2):,}
...
• H2 & H3: {len((h2 & h3) - h1):,}"""
    ...
    # Right: Individual criteria sizes
    ax2.bar(['H1: Normal\\nLabs', 'H2: Referral\\nLoops', 'H3: Drug\\nPersistence'],
...
def create_demographic_comparison(data, output_dir):
    """Create improved demographic comparison with cleaned sex variable"""
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(14, 10))
    
    # Get exposed/unexposed groups
    ...
    ax2.set_title(f'Age Distribution - AND Logic\\n(Exposed: {len(and_exposed):,}, Unexposed: {len(and_unexposed):,})')
    ax2.legend()
    
    # 3. Sex distribution - OR Logic (percentages)
...
def create_criteria_intensity_analysis(data, output_dir):
    ...
    if len(data_h1) > 0:
        ...
        ax1.axvline(data_h1['normal_lab_count'].mean(), color='darkred', linestyle=':', 
                    label=f'Mean: {data_h1["normal_lab_count"].mean():.1f}')
        ax1.legend()
    ...
    if len(data_h2) > 0:
        ...
        ax2.axvline(data_h2['symptom_referral_n'].mean(), color='darkcyan', linestyle=':',
                    label=f'Mean: {data_h2["symptom_referral_n"].mean():.1f}')
        ax2.legend()
    ...
    if len(data_h3) > 0:
        ...
        ax3.axvline(data_h3['drug_days_in_window'].mean(), color='darkgreen', linestyle=':',
                    label=f'Mean: {data_h3["drug_days_in_window"].mean():.1f}')
        ax3.legend()
...
def create_power_analysis_comparison(stats, output_dir):
    """Create improved power analysis comparison"""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    
    # Calculate MDE values
    or_n_per_group = min(stats['or_exposed'], stats['or_unexposed'])
    and_n_per_group = min(stats['and_exposed'], stats['and_unexposed'])
    
    or_mde = 2.8 / np.sqrt(or_n_per_group)
    and_mde = 2.8 / np.sqrt(and_n_per_group) if and_n_per_group > 0 else np.inf
    
    # Left panel: MDE comparison
    bars = ax1.bar(['OR Logic', 'AND Logic'], [or_mde, and_mde], 
                    color=['#4ECDC4', '#FF6B6B'], alpha=0.8)
    
    for bar, val in zip(bars, [or_mde, and_mde]):
        height = bar.get_height()
    ...
    ax1.set_ylim(0, max(0.25, and_mde * 1.2) if and_mde != np.inf else 0.25)
    ...
    # Right panel: Sample sizes with log scale
    bars2 = ax2.bar(['OR Logic', 'AND Logic'], [stats['or_exposed'], stats['and_exposed']], 
                    color=['#4ECDC4', '#FF6B6B'], alpha=0.8)
    
    for bar, val in zip(bars2, [stats['or_exposed'], stats['and_exposed']]):
        height = bar.get_height()
    ...
    ax2.text(0.5, 0.05, f'Ratio: {stats["ratio"]:.0f}x difference', 
...
def generate_summary_report(data, stats, overlap_stats, demo_stats, power_stats, output_dir):
    ...
    summary = {
        ...
        'recommendations': {
            'power_consideration': 'AND logic is severely underpowered for causal inference' if power_stats['and_mde'] > 0.2 else 'Both approaches have adequate power',
            'clinical_validity': 'AND logic provides more specific phenotype but limited sample',
            'suggested_approach': 'Use OR logic for primary analysis with AND logic sensitivity analysis' if power_stats['and_mde'] > 0.2 else 'Either approach viable depending on clinical priorities'
        }
    }
...
\\begin{itemize}
\\item OR Logic: ''' + f"{summary['or_logic']['female_pct_exposed']:.1f}" + r'''\\% female among exposed
\\item AND Logic: ''' + f"{summary['and_logic']['female_pct_exposed']:.1f}" + r'''\\% female among exposed
\\item Mean age higher in AND logic exposed group
\\end{itemize}
...
\\begin{figure}[H]
\\centering
\\includegraphics[width=0.9\\textwidth]{power_analysis_comparison.png}
\\caption{Statistical power comparison between approaches}
\\end{figure}

''' + (r'''\\textcolor{alertred}{\\textbf{Critical Finding}}: The AND logic approach has a minimum detectable effect size of ''' + str(summary['and_logic']['minimum_detectable_effect']) + r''', indicating severe power limitations.}''' if summary['and_logic']['minimum_detectable_effect'] == 'Infinity' or (isinstance(summary['and_logic']['minimum_detectable_effect'], float) and summary['and_logic']['minimum_detectable_effect'] > 0.2) else '') + r'''
...
def main():
    ...
    print(f"  OR exposed: {demo_stats['or_female_pct_exposed']:.1f}%")
    print(f"  AND exposed: {demo_stats['and_female_pct_exposed']:.1f}%")
